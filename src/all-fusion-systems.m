...
function ComputerAllBorelPairs(S, p, OutFSOrders, ProtoEssentialAutClasses)
...
function ExpandProtoEssentials(B,S, ProtoEssentialAutClasses, S_centrics)
    ...
    // Note these really are protoessentials, no S
    ProtoEssentials:=[];
    for x in ProtoEssentialAutClasses do
        ...
        if #PNew ne 0 then 
            for j := 1 to #PNew do
                ...
                y`autF:=[];
                for AP in x`autF do
                    y`autF := Append(y`autF, 
                        sub<y`autogrp|[Inverse(Rx[Index(Xx,y)])*gamma*Rx[Index(Xx,y)]: gamma in Generators(AP)]>);
                end for;
            end for; 
        end if;
    end for;
...
intrinsic AllFusionSystems(S::Grp:SaveEach:=false,Printing:=false,OutFSOrders:=[],OpTriv:=true,pPerfect:= true)-> SeqEnum
    ...
    // Now calculate the Borel pairs
    BorelsandS := ComputerAllBorelPairs(S, p, OutFSOrders, ProtoEssentialAutClasses);
    if Printing then 
    ...
    count:=0;
    // Iterate over the Borels, this really needs improving
    for Bor in BorelsandS do
       count := count+1; 
        ...
        // Get all our automorphism groups set up
        MakeAutos(S);    
        Bbar, bar:= B/Centre(S);
        ...
        if OpTriv then 
            if #ProtoEssentials eq 2 and p ge 5 and #S lt p^(p+3) then 
                ...
                PC:= Core(S,P2);
                if (IsConjugate(B,P1,NSP2) and Index(NSP2,P2) eq p and Index(S,NSP2) eq p 
                    and Index(P2,PC) eq p and IsCharacteristic(NSP2,PC) 
                    and IsNormal(B,DerivedSubgroup(P1))) then        
                    ProtoEssentials:=[P2];
                end if;
            end if;    
        end if; //OpTriv
        ...
        // If we have just one essential and it is normal in B then O_p(F) neq 1
        if OpTriv and #ProtoEssentials eq 1 and IsNormal(B,ProtoEssentials[1]) then 
            continue Bor; 
        end if; 
          

        if Printing then 
            print "There are", #ProtoEssentials, "proto-essential subgroups before the extension test.\nThey have orders", 
            Explode([#ProtoEssentials[i]: i in [1..#ProtoEssentials]]);
        end if;
