...
|Calculate $\mathfrak{hyp}(\mathcal{F})$ | Calculate the hyperfocal subgroup, of course we could calculate the focal subgroup from this so once done perhaps do some testing to see which is quicker, I imagine it'd be slower | Done in [branch](https://github.com/willendeavor/fusion-systems/tree/dev-add-fusion-subsystems) |
